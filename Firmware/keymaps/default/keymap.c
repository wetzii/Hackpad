...
const uint16_t PROGMEM keymaps[][MATRIX_ROWS][MATRIX_COLS] = {
    ...
    // -----------------------------------------------------------------------
    // Layer 2: CUSTOM (Toggle via FN + SW4)
    // -----------------------------------------------------------------------
    [_CUSTOM] = LAYOUT_4x4(
...
bool oled_task_user(void) {
    ...
    switch (get_highest_layer(layer_state)) {
        case _NUMPAD:
            oled_write_P(PSTR("SW16=FN         "), false);
            break;
        case _FN:
            oled_write_P(PSTR("SW4=CUSTOM      "), false);
            break;
        case _CUSTOM:
            oled_write_P(PSTR("SW4=NUMPAD      "), false);
            break;
        default:
            oled_write_P(PSTR("                "), false);
    }
